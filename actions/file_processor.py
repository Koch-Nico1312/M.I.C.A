"""
file_processor.py — M.I.C.A Universal File Processor
...
def _gemini_client():
    return get_routed_model(use_cache=False)
...
def _process_video(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    def _ffmpeg_available() -> bool:
        try:
            subprocess.run(["ffmpeg", "-version"], capture_output=True, timeout=3, encoding="utf-8")
            return True
        except Exception:
            return False
    ...
    if action == "trim":
        ...
        try:
            ...
            subprocess.run(cmd, capture_output=True, timeout=600, encoding="utf-8")
