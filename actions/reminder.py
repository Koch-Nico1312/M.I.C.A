...
try:
    ...
    notification.notify(title="M.I.C.A Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("M.I.C.A Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        subprocess.run(["msg", "*", "/TIME:30", message], check=False, encoding='utf-8', errors='ignore')
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "M.I.C.A Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by M.I.C.A reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str, script_path: Path, message: str) -> str:
    ...
    xml_content = (
        ...
        "  <RegistrationInfo><Description>M.I.C.A Reminder</Description></RegistrationInfo>\n"
...
def _schedule_mac(target_dt: datetime, task_name: str, script_path: Path) -> str:
    ...
    label = f"com.mica.reminder.{task_name}"
...
def reminder(
    ...
    task_name = f"MICAReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
