...
def computer_settings(
    ...
    try:
        ...
        # Destructive system action check (shutdown/restart)
        if action in ("shutdown", "restart") and not allow_destructive and profile != "admin":
            is_allowed, msg = check_action(profile, action, params)
            if not is_allowed:
                return msg

        # Standard check for all actions
        is_allowed, msg = check_action(profile, action, params)
        if not is_allowed:
            return msg
    except Exception as se:
        print(f"[Settings Safety] Check failed: {se}")
