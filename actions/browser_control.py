...
class _BrowserSession:
    """
    Complete session for a browser instance.
    All browsers are launched with real profiles using launch_persistent_context.
    """
    ...
    async def _launch(self) -> None:
        ...
        if self._spec is None:
            raise RuntimeError(f"'{self.browser_name}' is not supported on this platform ({_OS}).")
    ...
    async def go_to(self, url: str) -> str:
        ...
        async def _do_goto(p: Page) -> str:
            ...
            try:
                ...
                logger.error(f"goto exception (non-fatal): {e}")
            return p.url
        ...
        if result_url in ("about:blank", "", None, prev_url) and prev_url in (
            ...
            logger.warning(f"Still blank after goto — retrying on new tab: {url}")
            try:
                new_page = await self._context.new_page()
                self._page = new_page
                result_url = await _do_goto(new_page)
            except Exception as e:
                logger.error(f"New-tab retry failed: {e}")
...
class _SessionRegistry:
    ...
    def close_one(self, browser_name: str) -> str:
        ...
        if sess:
            ...
            if self._active_browser == browser_name:
                self._active_browser = ""
            logger.info(f"Closed browser: {browser_name}")
    ...
    def close_all(self) -> str:
        ...
        logger.info(f"Closed all browsers: {', '.join(names) if names else 'none'}")
