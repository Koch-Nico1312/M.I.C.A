...
def _launch_windows(app_name: str) -> bool:
    ...
    if shutil.which(app_name) or shutil.which(app_name.split(".")[0]):
        try:
            subprocess.Popen(
                app_name, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            time.sleep(1.5)
            return True
...
def _launch_linux(app_name: str) -> bool:
    ...
    if binary:
        try:
            subprocess.Popen(
                [binary], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, encoding="utf-8"
            )
            time.sleep(1.0)
            return True
        except Exception:
    ...
    try:
        subprocess.run(["xdg-open", app_name], capture_output=True, timeout=5, encoding="utf-8")
        return True
    except Exception:
        pass
    ...
    for desktop_name in [
        ...
        try:
            result = subprocess.run(
                ["gtk-launch", desktop_name], capture_output=True, timeout=5, encoding="utf-8"
            )
            if result.returncode == 0:
                return True
        except Exception:
