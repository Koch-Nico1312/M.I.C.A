...
if WATCHDOG_AVAILABLE:
    class VaultFileHandler(FileSystemEventHandler):
        """Event handler for Obsidian vault file changes"""
        ...
        def on_modified(self, event):
            if not event.is_directory:
                self.metrics.start_operation("vault_file_modified")
                self.metrics.end_operation(
                    "vault_file_modified", {"file": event.src_path, "event_type": "modified"}
                )

        def on_created(self, event):
            if not event.is_directory:
                self.metrics.start_operation("vault_file_created")
                self.metrics.end_operation(
                    "vault_file_created", {"file": event.src_path, "event_type": "created"}
                )

        def on_deleted(self, event):
            if not event.is_directory:
                self.metrics.start_operation("vault_file_deleted")
                self.metrics.end_operation(
                    "vault_file_deleted", {"file": event.src_path, "event_type": "deleted"}
                )
else:
...
class ObsidianVault:
    ...
    def _setup_file_watching(self):
        """Setup event-based file watching for the vault"""
        perf_flags = get_performance_flags()

        if not perf_flags.is_enabled("event_file_watching"):
            return

        if not WATCHDOG_AVAILABLE:
            print("[Obsidian] ⚠️ Watchdog not available, event-based file watching disabled")
            return
        ...
        try:
            ...
            self.observer.start()
            print("[Obsidian] ✅ Event-based file watching enabled")
        except Exception as e:
            print(f"[Obsidian] ⚠️ Could not setup file watching: {e}")
            self.observer = None
    ...
    def stop_file_watching(self):
        ...
        if self.observer:
            ...
            self.observer.join()
            self.observer = None
            print("[Obsidian] File watching stopped")

    def create_conversation_note(
        self, conversation: Dict[str, Any], related_notes: List[str] = None
    ) -> str:
    ...
    def _build_note_content(self, conversation: Dict[str, Any], timestamp: datetime) -> str:
    ...
    def link_notes_with_ai(self, conversation_summary: str, api_key: str = None) -> List[str]:
    ...
    def end_conversation(self, summary: str = None) -> str:
        ...
        if summary:
            ...
            if self.current_conversation["key_points"]:
                self.current_conversation["summary"] = ". ".join(
                    self.current_conversation["key_points"]
                )
            else:
                self.current_conversation["summary"] = (
                    f"Conversation from {self.current_conversation['start_time'].strftime('%Y-%m-%d %H:%M')}"
                )

        # Find related notes
        related_notes = self.vault.link_notes_with_ai(self.current_conversation["summary"])

        # Create note
        note_title = self.vault.create_conversation_note(self.current_conversation, related_notes)
