...
class MultiAgentOrchestrator:
    ...
    def _determine_agent_type(self, goal: str) -> AgentType:
        ...
        if any(keyword in goal_lower for keyword in ["research", "search", "find information", "investigate", "analyze data"]):
            return AgentType.DEEP_RESEARCH
        
        # Data analysis tasks
        elif any(keyword in goal_lower for keyword in ["analyze", "data", "statistics", "calculate", "spreadsheet", "table"]):
            return AgentType.DATA_ANALYST
        
        # Code-related tasks
        elif any(keyword in goal_lower for keyword in ["code", "programming", "debug", "implement", "function", "script"]):
            return AgentType.CODE_EXPERT
        
        # Web scraping tasks
        elif any(keyword in goal_lower for keyword in ["scrape", "crawl", "extract from web", "download from"]):
            return AgentType.WEB_SCRAPER
        
        # File management tasks
        elif any(keyword in goal_lower for keyword in ["file", "folder", "directory", "organize", "delete", "move"]):
    ...
    def execute_all_async(self, task_ids: List[str], timeout: float = 300.0) -> Dict[str, AgentResult]:
        """
