...
def test_push_to_talk_gates_audio_capture():
    ...
    assert not voice.should_capture_audio(muted=False, jarvis_speaking=False)

    voice.configure(input_mode="push_to_talk", push_to_talk_active=True)
    assert voice.should_capture_audio(muted=False, jarvis_speaking=False)
    assert not voice.should_capture_audio(muted=True, jarvis_speaking=False)
    assert not voice.should_capture_audio(muted=False, jarvis_speaking=True)
