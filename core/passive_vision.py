...
class VisionMemory:
    ...
    def add(self, timestamp: datetime, ocr_text: str, image_hash: str, metadata: Dict[str, Any] = None):
...
class PassiveVision:
        ...
        while self.running:
            try:
                ...
                if image is not None:
                    ...
                    print(f"[PassiveVision] 📸 Captured (OCR: {len(ocr_text)} chars)")
    ...
    def query_memory(self, question: str) -> str:
        ...
        if matches:
            return f"Found {len(matches)} relevant entries in visual memory from the last 5 minutes."
        
        return f"I have {len(recent)} recent visual memories, but none directly match your query, sir."
    
    def save_memory_to_disk(self):
        """Save current memory to disk"""
        try:
            memory_file = self.storage_path / f"vision_memory_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
