...
class CacheManager:
    ...
    def _initialize_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_llm_prompt_hash ON llm_responses(prompt_hash)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_llm_expires ON llm_responses(expires_at)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_text_hash ON embeddings(text_hash)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_expires ON embeddings(expires_at)")
    ...
    def set(self, key: str, value: Any, ttl_hours: Optional[float] = None, metadata: Optional[Dict] = None) -> bool:
        """
    ...
    def clear_expired(self) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("DELETE FROM llm_responses WHERE expires_at IS NOT NULL AND expires_at < ?", (time.time(),))
                llm_count = cursor.rowcount
                
                # Clear expired embeddings
                cursor.execute("DELETE FROM embeddings WHERE expires_at IS NOT NULL AND expires_at < ?", (time.time(),))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("SELECT COUNT(*) FROM cache WHERE expires_at IS NOT NULL AND expires_at < ?", (time.time(),))
    ...
    def cache_embedding(self, text: str, model: str, embedding: List[float], ttl_hours: float = 720.0) -> bool:
        """
        Cache an embedding.
