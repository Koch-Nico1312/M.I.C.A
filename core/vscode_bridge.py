...
class VSCodeBridge:
    ...
    async def edit_file(self, file_path: str, edits: list) -> Dict:
        ...
        return await self.send_command('edit_file', {
            'file_path': file_path,
            'edits': edits
        })
    
    async def refactor_code(self, file_path: str, pattern: str, 
                           replacement: str) -> Dict:
        """Refactor code in VS Code"""
        return await self.send_command('refactor', {
            'file_path': file_path,
            'pattern': pattern,
    ...
    async def set_cursor_position(self, line: int, character: int) -> Dict:
        ...
        return await self.send_command('set_cursor', {
            'line': line,
            'character': character
        })
    ...
    def sync_refactor(self, file_path: str, pattern: str, 
                     replacement: str) -> Dict:
        ...
        try:
            result = loop.run_until_complete(
                self.refactor_code(file_path, pattern, replacement)
            )
            return result
        finally:
            loop.close()
