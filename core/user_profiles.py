...
class UserProfiles:
    ...
    def _init_db(self) -> None:
        ...
        with _lock:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_voice_profile ON voice_profiles(profile_id)"
            )
    ...
    def create_voice_profile(
        self, user_id: str, audio_features: Optional[Dict[str, float]] = None
    ) -> VoiceProfile:
    ...
    def update_voice_profile(self, profile_id: str, audio_features: Dict[str, float]) -> bool:
        ...
        with _lock:
            ...
            cursor.execute(
                ...
                WHERE profile_id = ?
            """,
                (json.dumps(merged_features), datetime.now().isoformat(), sample_count, profile_id),
            )
    ...
    def identify_speaker(self, audio_features: Dict[str, float]) -> Optional[UserProfile]:
        ...
        if best_match:
            ...
            if profile:
                print(
                    f"[User Profiles] 🔊 Identified speaker: {profile.name} (confidence: {best_score:.2f})"
                )
...
__all__ = ["UserProfiles", "UserProfile", "VoiceProfile", "get_user_profiles"]
